...
| File | Tests | Covers |
|------|-------|--------|
| `security_hardening_test.dart` | 11 | **AES random IV (#161):** encryptString produces different output for same input, encryptString/decryptString round-trip with random IV, decryptString rejects short data. **URL protocol support (#170):** SupabaseSettings.isConfigured accepts both HTTP and HTTPS URLs, SupabaseSettings.isHttpUrl detects insecure HTTP URLs. **Credential encryption (#153):** Supabase email/password encrypted in serialized map, credentials survive round-trip, backward compat with unencrypted settings, empty credentials not encrypted |
...
| Supabase settings | Covered | Model serialization, auto-sync fields, isConfigured (HTTP and HTTPS both accepted), isHttpUrl detection (#170), lastAutoSyncDateTime, credential encryption at rest |
...
| Security hardening | Covered | Random IV for all AES methods, URL protocol support (HTTP + HTTPS both accepted, #170), credential encryption at rest, backward compat |
