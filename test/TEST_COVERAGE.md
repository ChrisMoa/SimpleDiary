...
| `models/note_attachment_test.dart` | 13 | NoteAttachment construction (all fields, auto UUID, unique IDs), `copyWith` (preserves unchanged, updates all), `toMap`/`fromMap` round-trip (all keys, missing fileSize defaults to 0), `toJson`/`fromJson` round-trip (valid JSON, data preserved), LocalDb map round-trip (same keys as toMap), `primaryKeyValue` returns id |
...
| File | Tests | Covers |
|------|-------|--------|
| `export_data_test.dart` | 17 | **ExportMetadata:** `toMap`/`fromMap`, `toJson`/`fromJson`, null handling. **ExportData.isNewFormat:** new format detection, legacy array, invalid JSON, missing keys. **Unencrypted parsing:** data as list, data as string, `fromJson`. **Encrypted:** decrypt with correct password, missing password, missing salt, wrong password. **Round-trips:** unencrypted, encrypted |
