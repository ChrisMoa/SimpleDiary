...
import 'package:SimpleDiary/services/database_services/notes_local_db.dart';
...
final notesLocalDataProvider = StateNotifierProvider<NotesLocalDataProvider, List<Note>>((ref) {
...
final notesOfSelecteDayProvider = Provider((ref) {
  ...
  LogWrapper.logger.t('updates selectedDate to: ${Utils.toDateTime(selectedDate)}');

  return notes.where((element) {
    return element.from.year == selectedDate.year && element.from.month == selectedDate.month && element.from.day == selectedDate.day;
  }).toList();
});
